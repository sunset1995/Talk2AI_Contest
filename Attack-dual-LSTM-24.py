...
params['n_layers'] = 2
params['forget_bias'] = 1.0 # 0.0

# Training Parameters
params['learning_rate'] = 1e-3
params['decay_learning_rate'] = 0.8
params['decay_times_no_improve'] = 10
params['keep_prob_train'] = 0.5
...
def get_valid_loss_accuracy(sess):
    ...
    print('Valid loss = %.5f, accuracy = %.5f, elapsed time %.f' % (valid_loss, valid_accuracy, time.time() - start_time), flush=True)
    return valid_loss
...
with tf.Session() as sess:
    ...
    #saver.restore(sess, 'models/dual_lstm_21/best/model.ckpt')
    ...
    times_no_improve = 0
    decay_times_no_improve = params['decay_times_no_improve']
...
saver = tf.train.Saver()
sess = tf.Session()
saver.restore(sess, 'models/dual_lstm_13/best/model.ckpt')
...
# with open('answer/exp/exp_13_best_sigmoid_cMr.txt', 'w') as f:
#     f.write('id,ans\n')
#     for i, a in enumerate(my_ans):
#         f.write('%d,%d\n' % (i+1, a))
...
# with open('answer/exp/exp_13_best_cossim_cMr.txt', 'w') as f:
#     f.write('id,ans\n')
#     for i, a in enumerate(my_ans):
#         f.write('%d,%d\n' % (i+1, a))
...
# with open('models/word2vec/dual-lstm-13-best.txt', 'w') as f:
#     f.write('%d %d\n' % now_emb.shape)
